...
public class OrderServices
{
    ...
    public List<orders> getOrder()
    {
        List<orders> li=new List<orders>();
        con.Open();
        SqlCommand cmd = new SqlCommand("select * from orders,schedules,rooms,bills,services,categories where orders.schedule_id=schedules.schedule_id and schedules.room_id=rooms.room_id and orders.bill_id=bills.bill_id and orders.service_id=services.service_id and services.cat_id=categories.cat_id and schedules.schedule_status=0", con);
        SqlDataReader rd = cmd.ExecuteReader();
        while (rd.Read())
        {
    ...
    {
        con.Open();
        SqlCommand cmd = new SqlCommand("insert into orders values(@schedule_id,@service_id,@quantity,@order_price,@bill_id)", con);
    ...
    //cap nhat gia dich vu trong hoa don
